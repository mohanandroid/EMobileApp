...
namespace eCups.Droid
{
    public class GesturesRecogniser : ViewRenderer
    {
        ...
        protected override void OnElementChanged(ElementChangedEventArgs<Xamarin.Forms.View> e)
        {
            ...
            var mobiusGestures = (Gestures)e.NewElement;
            ...
            if (e.OldElement == null)
            {
                ...
                _listener.OnTap = (x, y) => mobiusGestures.OnTap(x, y);

                if (mobiusGestures.LongTapEnabled)
                {
                    _listener.OnLongTap = () => mobiusGestures.OnLongTap();
                }

                _listener.OnTouchBegan = (x, y) => mobiusGestures.OnTouchBegan(x, y);
                _listener.OnTouchEnded = (x, y) => mobiusGestures.OnTouchEnded(x, y);

                if (mobiusGestures.SwipeEnabled)
                {
                    _listener.OnSwipeLeft = () => mobiusGestures.OnSwipeLeft();
                    _listener.OnSwipeRight = () => mobiusGestures.OnSwipeRight();
                    _listener.OnSwipeUp = () => mobiusGestures.OnSwipeUp();
                    _listener.OnSwipeDown = () => mobiusGestures.OnSwipeDown();
                }

                if (mobiusGestures.DragEnabled)
                { 
                    _listener.OnDrag = (x, y) => mobiusGestures.OnDrag(x, y);
                }
            }
        }
